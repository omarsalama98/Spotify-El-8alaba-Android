...
/**
 * @author Ali Adel Media playback service to play music from background even when activity is not
 * visible
 */
public class MediaPlaybackService extends Service implements MediaPlayer.OnPreparedListener,
    ...
    /**
     * utility function to destroy media player and stop handler thread
     */
        ...
        request.enqueue(new Callback<TrackList>() {
            ...
            public void onResponse(Call<TrackList> call, Response<TrackList> response) {
                ...
                if (!response.isSuccessful()) {
                    Toast.makeText(getApplicationContext(), "failed, token expired",
            ...
            public void onFailure(Call<TrackList> call, Throwable t) {
                Toast.makeText(getApplicationContext(), "fail: " + t.getMessage(),
                        Toast.LENGTH_LONG).show();
            }
        });
    }
     ...
     *
     * @return true : network connection available false : no network connection
     */
     ...
     *
     * @return true if succeeds to skip : there is a next song false if fails to skip : last song in
     * list
     */
    ...
    /**
     * when song finishes playing this is called and skip to next song if available if not then stop
     * playing
     *
